    ...
    <div className="container" style={{ padding: '2.5rem 1.5rem', maxWidth: 880 }}>
      ...
      <div className="no-print glass-panel" style={{ padding: '2rem', textAlign: 'center', marginBottom: '2rem', background: '#ffffff', border: '1px solid var(--border-color)' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem' }}>
          Thank you for shopping with TechCore. Your Invoice number is <strong style={{ color: 'var(--primary-blue)' }}>#{order.id}</strong>.
        </p>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #ea580c', paddingBottom: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1rem 1.25rem', minWidth: '220px', textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '0.84rem', fontWeight: 700, color: '#334155' }}>
              Invoice No : <span style={{ color: '#0f172a' }}>#{order.id}</span>
            </div>
            <div style={{ fontSize: '0.82rem', color: '#64748b', marginTop: '0.2rem' }}>
              Date : {order.createdAt}
            </div>
            <div style={{ fontSize: '0.82rem', color: '#64748b', marginTop: '0.2rem' }}>
              Payment : <span style={{ fontWeight: 700, color: '#ea580c' }}>{order.paymentMethod}</span> ({order.paymentStatus})
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '1.75rem', background: '#f8fafc', padding: '1.25rem', borderRadius: '8px', border: '1px solid #f1f5f9' }}>
          <div>
            ...
            <div style={{ fontSize: '1rem', fontWeight: 800, color: '#0f172a' }}>{order.customer.name}</div>
            <div style={{ fontSize: '0.85rem', color: '#475569', marginTop: '0.2rem' }}>Phone: {order.customer.phone}</div>
            <div style={{ fontSize: '0.85rem', color: '#475569' }}>Email: {order.customer.email}</div>
          </div>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#0f172a' }}>{order.customer.address}</div>
            <div style={{ fontSize: '0.85rem', color: '#475569', marginTop: '0.2rem' }}>{order.customer.city} ({order.customer.zone})</div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', fontSize: '0.9rem' }}>
            ...
            {order.discount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.3rem 0', color: '#16a34a' }}>
                <span>Discount:</span>
                <span style={{ fontWeight: 700 }}>-৳{order.discount.toLocaleString()}</span>
              </div>
            )}

            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.3rem 0', color: '#475569' }}>
              <span>Delivery Fee:</span>
              <span style={{ fontWeight: 700, color: '#0f172a' }}>৳{order.deliveryFee}</span>
            </div>
